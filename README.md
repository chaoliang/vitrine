...
商家一次给一堆货的时候，`split` 把商品册拆成一支支能跑的配置：

```bash
vitrine split configs/商品册.json --dry-run   # 先看怎么分的，一个文件都不写
vitrine split configs/商品册.json --out configs/
vitrine batch configs/ --producer "你的公司"
```

**分片是按穿搭分的，不是按顺序切的。** 一支片里凑成一套能同时穿的搭配，
会打架的两件（中长裙配过膝靴——靴筒口落在裙摆里，腿的线条会断）永远不同片；
同部位的多件落在同一支里当换装序列，所以只卖半裙的店会拿到「一支四条裙子」，
那不是妥协，那正是它该有的样子；每个商品只出现一次，一件都不会漏。

商品册本身允许自相冲突（它就是一份货单，不是一支片），
所以 `split` 用「按商品册读」的方式载入它 —— 拆开正是这条命令的职责。
拆出来的每一份都会被当成成片重新校验一遍，站不住就当场报错。
...
不碰 GPU，一秒跑完。里面每一条断言都是真实翻过的车：
只卖半裙的店拿到十支单品片、中长裙和过膝靴渲进了同一支、连衣裙和半裙撞在一起、
「短筒配高跟」这种同时出现两个长度词的描述靠字典顺序决定判定。
